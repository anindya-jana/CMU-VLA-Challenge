...
class StateMachine:
    ...
    def create_voxel_map(self):
        try:
            ...
            # start navigation in a separate thread so we can enforce a timeout
            nav_result = {'success': False}
            nav_done = threading.Event()

            def nav_thread_fn():
                try:
                    res = controller.run(waypoint_list)
                    nav_result['success'] = bool(res)
                except Exception as e:
                    # don't let navigation exceptions crash the state machine
                    self.log(f"Navigation thread error: {e}")
                    nav_result['success'] = False
                finally:
                    nav_done.set()
            ...
            # wait up to 60 seconds for the navigation to finish
            timeout_seconds = 500
            finished_in_time = nav_done.wait(timeout=timeout_seconds)

            if finished_in_time:
                navigation_successful = nav_result['success']
                if navigation_successful:
                    self.log("Navigation completed successfully within timeout.")
                    # use collected voxel map
                    self.voxel_map = voxel_map_handler.voxel_map
                    try:
                        voxel_map_handler.save_voxel_map(self.voxel_map)
                        self.log("Voxel map saved.")
                    except Exception as e:
                        self.log(f"Failed to save voxel map: {e}")
                else:
                    self.log("Navigation finished but reported unsuccessful. Proceeding to question phase with current voxel map (if any).")
                    self.voxel_map = voxel_map_handler.voxel_map
                    try:
                        if self.voxel_map:
                            voxel_map_handler.save_voxel_map(self.voxel_map)
                            self.log("Partial voxel map saved.")
                    except Exception as e:
                        self.log(f"Failed to save partial voxel map: {e}")
            else:
                # timed out
                self.log(f"Navigation timed out after {timeout_seconds} seconds. Proceeding to question phase.")
                # best-effort: try to stop controller if it supports stop/abort
                try:
                    if hasattr(controller, "stop") and callable(controller.stop):
                        controller.stop()
                        self.log("Called controller.stop() after timeout.")
                except Exception as e:
                    self.log(f"Error calling controller.stop(): {e}")

                # take whatever voxel map we have so far
                try:
                    self.voxel_map = voxel_map_handler.voxel_map
                    if self.voxel_map:
                        voxel_map_handler.save_voxel_map(self.voxel_map)
                        self.log("Partial voxel map saved after timeout.")
                    else:
                        self.log("No voxel map available after timeout.")
                except Exception as e:
                    self.log(f"Error saving partial voxel map after timeout: {e}")

            # regardless of navigation outcome, go to question phase after the timeout/finish
            self.state = State.ASK_QUESTION
            self.log("Please type in the Question in the box")
        except Exception as e:
