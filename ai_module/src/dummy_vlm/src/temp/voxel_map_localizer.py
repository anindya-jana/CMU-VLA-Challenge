...
class VoxelMapLocalizer():
    ...
    def _init_model(self):
        """
        Initialize SigLIP2 text model with local-cache-first strategy and offline fallback.
        Keeps OwlViT object detection logic untouched.
        """
        # Build candidate list (env override first, then fallbacks)
        _env = os.getenv("SIGLIP2_MODEL_ID", "").strip() or None
        ...
        for mid in candidates:
            try:
                ...
                self.clip_model = AutoModel.from_pretrained(mid, local_files_only=True).to(self.device)
                self._infer_dim()
                try:
                    rospy.loginfo(f"VoxelMapLocalizer: SigLIP2 loaded from local cache: {mid}")
                except Exception:
                    pass
                self._offline_semantics = False
                return
        ...
        for mid in candidates:
            try:
                ...
                self.clip_model = AutoModel.from_pretrained(mid).to(self.device)
                self._infer_dim()
                try:
                    rospy.loginfo(f"VoxelMapLocalizer: SigLIP2 loaded online: {mid}")
                except Exception:
                    pass
                self._offline_semantics = False
                return
        ...
        try:
            rospy.logwarn(f"VoxelMapLocalizer: SigLIP2 unavailable; enabling offline semantics. Reason: {last_err}")
        except Exception:
            pass
    ...
    def calculate_clip_and_st_embeddings_for_queries(self, queries):
        ...
        with torch.no_grad():
            ...
            if hasattr(self.clip_model, "get_text_features"):
                try:
                    feats = self.clip_model.get_text_features(**tensor_inputs)
                except Exception:
                    feats = None
